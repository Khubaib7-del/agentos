...
- **Plugin update doesn't pick up new commands** — updates only apply when the plugin's version number increases (we bump it on every plugin change). Run `/plugin marketplace update thruline`, then `/plugin update thruline@thruline`, then start a new session. Last resort: uninstall plugin → remove marketplace → re-add → reinstall.
